...
class ActionLayer(BaseActionLayer):
    ...
    def _inconsistent_effects(self, actionA, actionB):
        ...
        # get effects
        effectsOfA = self.children[actionA]
        effectsOfB = self.children[actionB]

        # test effects against each other
        for effect in effectsOfA:
            if ~effect in effectsOfB:
                return True
    ...
    def _interference(self, actionA, actionB):
        ...
        """
        # TODO: implement this function
        # get effects and preconditions
        effectsOfA = self.children[actionA]
        effectsOfB = self.children[actionB]
        preA = self.parents[actionA]
        preB = self.parents[actionB]

        # go through effects of A and see if it negates precondition of B
        for effect in effectsOfA:
            if ~effect in preB:
                return True

        # go through effects of B and see if it negates precondition of A 
        for effect in effectsOfB:
            if ~effect in preA:
                return True
    ...
    def _competing_needs(self, actionA, actionB):
        ...
        # go through each combo and test if mutex
        for preconA in preA:
            for preconB in preB:
...
class LiteralLayer(BaseLiteralLayer):
    ...
    def _inconsistent_support(self, literalA, literalB):
        ...
        """
        # TODO: implement this function
        # get pre actions for both literals
        preActionsOfA = self.parents[literalA]
        preActionsOfB = self.parents[literalB]

        # compare pre actions and check if mutex in parent layer
        for preActionA in preActionsOfA:
            for preActionB in preActionsOfB:
                if not self.parent_layer.is_mutex(preActionA, preActionB):
                    return False
...
class PlanningGraph:
    ...
    def h_levelsum(self):
        ...
        """
        # TODO: implement this function
        # get goals and initialize costs
        costs = []
        ...
        # while goals remaining
        while not self._is_leveled and goals:

            # for each goal in the list while making new copy every time
            for goal in goals[:]:

                # if goal is there then add the cost
            ...
            # extend while there are goals remaining
            if goals:
                self._extend()

        # add costs and return
        return sum(costs)
    ...
    def h_maxlevel(self):
        ...
        while not self._is_leveled and goals:
            ...
            # go through goals and get new list copy each iteration
            for goal in goals[:]:

                # if goal is there, add cost
                if goal in self.literal_layers[i]:
                    costs.append(i)
            ...
            # extend while goals remaining
            if goals:
                self._extend()

        # get the max from costs
        return max(costs)
    ...
    def h_setlevel(self):
        ...
        while not self._is_leveled:
            ...
            goals_met = True
            contains_mutex = False
            layer = self.literal_layers[i]

            # see if all goals are met in layer
            for goal in goals:
                if goal not in layer:
                    goals_met = False
                    break
            
            # if all goals met, check them against each other for mutex
            if goals_met:
                for goal1 in goals:
                    for goal2 in goals:

                        # if not same goal
                        if goal1 is not goal2:
                    ...
                    # if mutex found, exit loop
                    if contains_mutex:
                        break

            # return if no mutex and all goals met
            if not contains_mutex and goals_met:
                return i
            else:
                # extend if mutex or goals remaining
                self._extend()
